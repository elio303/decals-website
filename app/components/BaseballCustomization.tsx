...
import styles from '@/app/components/BaseballCustomization.module.css';
...
import {
  calculateBasePriceBaseball,
  calculateShippingCostBaseball,
  calculateMoldCostBaseball,
} from '@/app/utils/calculate';
import { useFormContext } from '@/context/FormContext';
import { baseballConstants } from '../constants/constants';

const BaseballCustomization = () => {
  const { formInput, setFormInput } = useFormContext();

  // Calculations
  const bumperQuantity = formInput.bumperQuantity || 0;
  const baseSubTotal = calculateBasePriceBaseball(bumperQuantity);
  const totalShippingCost = calculateShippingCostBaseball(bumperQuantity);
  const moldSubTotal = calculateMoldCostBaseball();
  ...
  return (
    <div>
      <h1 className={styles.title}>Customize Your Baseball Bumper</h1>
      ...
      {/* Quantity Sections */}
      <SimpleQuantitySection title="Bumper Quantity" quantity={formInput.bumperQuantity} handleInputChange={handleInputChange} name='bumperQuantity' min={baseballConstants.minLogos} />
...
export default BaseballCustomization;
